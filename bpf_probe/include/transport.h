...
enum event_type
{
    ...
    __EVENT_MAX, // Keep this last
};

#define EVENT_MASK(event) (1UL << (event))
#define EVENT_BITMASK_ALL() (EVENT_MASK(__EVENT_MAX) - 1UL)

#define BPF_PROCESS_EVENTS_MASK ( \
      EVENT_MASK(EVENT_PROCESS_EXEC_ARG) \
    | EVENT_MASK(EVENT_PROCESS_EXEC_PATH) \
    | EVENT_MASK(EVENT_PROCESS_EXEC_RESULT) \
    | EVENT_MASK(EVENT_PROCESS_EXIT) \
    | EVENT_MASK(EVENT_PROCESS_CLONE) \
)

#define HIGH_WATERMARK_MASK (~(BPF_PROCESS_EVENTS_MASK) & EVENT_BITMASK_ALL())
