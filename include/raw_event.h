...
typedef struct _CB_EVENT_PROCESS_START {
  ...
  bool observed; // Flag to identify if the start was actually observed, or this
                 // fake
  char cmdLine[CB_MAX_CMDLINE_SIZE + 1];
...
typedef struct _CB_EVENT_NETWORK_CONNECT {
  ...
  char actual_server[PROXY_SERVER_MAX_LEN + 1];
...
typedef struct CB_EVENT {
  ...
  // @@REVIEW - possibly have a scratch strings space here unioned with a char*
  // in the event its overflowed. need a stringlength to signify the length of
  // used scratch space uint32_t eventSize;
  // char    strings[CB_MAX_CMDLINE_SIZE];
} *PCB_EVENT;
