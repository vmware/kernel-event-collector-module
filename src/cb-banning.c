...
typedef struct blacklist_entry_s {
    HashTableNode link;
    BL_TBL_KEY    key;
    uint64_t hash;
    uint64_t    device;
...
bool cbBanningInitialize(ProcessContext *context)
{
    ...
    g_banning_table = hashtbl_init_generic(context,
                                           ...
                                           "banning_cache",
                                           sizeof(BL_TBL_KEY),
                                           offsetof(BlacklistEntry, key),
                                           offsetof(BlacklistEntry, link),
...
bool cbSetBannedProcessInodeWithoutKillingProcs(ProcessContext *context, uint64_t device, uint64_t ino)
{
    ...
    bep = (BlacklistEntry *)hashtbl_alloc_generic(g_banning_table, context);
    if (bep == NULL)
    {
        return false;
    }
...
inline bool cbClearBannedProcessInode(ProcessContext *context, uint64_t device, uint64_t ino)
{
    ...
    bep = (BlacklistEntry *) hashtbl_del_by_key_generic(g_banning_table, &key, context);
    if (!bep)
    {
        return false;
    }
...
bool cbKillBannedProcessByInode(ProcessContext *context, uint64_t device, uint64_t ino)
{
    ...
    bep = (BlacklistEntry *) hashtbl_get_generic(g_banning_table, &key, context);
    if (!bep)
    {
