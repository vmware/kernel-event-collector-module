...
static void sched_process_exit_probe(struct task_struct *task);
#endif
...
bool task_initialize(ProcessContext *context)
{
    ...
    register_trace_sched_process_exit(sched_process_exit_probe, NULL);
#else
    register_trace_sched_process_fork(sched_process_fork_probe);
    register_trace_sched_process_exit(sched_process_exit_probe);
#endif
...
void task_shutdown(ProcessContext *context)
{
    ...
    unregister_trace_sched_process_exit(sched_process_exit_probe, NULL);
#else
    unregister_trace_sched_process_fork(sched_process_fork_probe);
    unregister_trace_sched_process_exit(sched_process_exit_probe);
#endif
}

#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
void sched_process_exit_probe(void *data, struct task_struct *task)
#else
void sched_process_exit_probe(struct task_struct *task)
#endif
{
