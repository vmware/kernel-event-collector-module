...
// These helper macros are derived from `list_for_each_entry_rcu`.  We use them to
//  the loop variables on the private stack.
...
#define HAS_MORE_TASKS(stack)  (&rcu_dereference((stack)->child)->sibling != &rcu_dereference((stack)->task)->children)
...
void enumerate_and_track_all_tasks(ProcessContext *context)
{
    ...
    do
    {
        ...
        // TODO: There is some way we can get stuck in an infinite loop here. Fix and remove MAX_ENUMERATE_LOOPS.
