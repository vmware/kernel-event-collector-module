...
ssize_t KF_LEN = sizeof(struct CB_EVENT); // This needs to be sizeof(whatever we store in queue)
...
int user_comm_send_event(struct CB_EVENT *msg, ProcessContext *context)
{
        ...
        try_to_gain_capacity(tx_queue)))
    {
        ...
        TRACE(DL_VERBOSE, "send_event_atomic %p %llu\n", msg, readyCount);
        msg = NULL;
    }
    cb_write_unlock(&dev_spinlock, context);
    ...
    device_read(struct file *f,  char __user *ubuf, size_t count, loff_t *offset)
{
    ...
    TRY_DO_MSG(count >= KF_LEN,
               { xcode = -ENOMEM; },
               DL_COMMS, "%s: size mismatch count=%ld KF_LEN=%ld\n", __func__, count, KF_LEN);
    ...
    TRY_DO_MSG(rc,
               { xcode = -ENOMEM; },
               DL_COMMS,
               "%s: failed to dequeue event\n", __func__);

    TRY_DO_MSG(msg,
               { xcode = -ENOMEM; },
               DL_COMMS,
               "%s: dequeud msg is NULL\n", __func__);
    ...
    if (rc)
    {
        TRACE(DL_COMMS, "%s: copy to user failed rc=%d\n", __func__, rc);
        len = -ENXIO;
    } else
    {
        *offset = 0;
        len = KF_LEN;
        TRACE(DL_COMMS, "%s: read=%ld qlen_pri0=%llu qlen_pri1=%llu\n", __func__, len, qlen_pri0, qlen_pri1);
    }
...
static int device_open(struct inode *inode, struct file *filp)
{
    ...
    TRACE(DL_INFO, "%s: attempting to connect to device from pid[%d]\n", __func__, context.pid);
    ...
    TRACE(DL_INFO, "%s: connected to device from pid[%d]\n", __func__, context.pid);
...
{
    TRACE(DL_INFO, "%s: releasing device from pid[%d]; reader_pid[%d]\n", __func__, getpid(current), atomic_read(&reader_pid));
...
static unsigned int device_poll(struct file *filp, struct poll_table_struct *pts)
{
    ...
    TRACE(DL_COMMS, "%s: msg queued qlen=%llu\n", __func__, qlen);
...
static long device_unlocked_ioctl(struct file *filep, unsigned int cmd_in, unsigned long arg)
{
    ...
    TRACE(DL_INFO, "%s: ioctl from pid[%d]\n", __func__, context.pid);

    // Only the connected process can send ioctls to this kernel module.
    if (!is_process_connected_reader(context.pid))
    {
        TRACE(DL_ERROR, "%s: Cannot process cmd=%d, process not authorized; pid[%d], reader-pid[%d]\n", __func__, cmd, context.pid, atomic_read(&reader_pid));
        return -EPERM;
    }

    if (!arg)
    {
        TRACE(DL_ERROR, "%s: arg null\n", __func__);
        return -ENOMEM;
    }

    if (!is_ioctl_allowed(moduleState, cmd))
    {
        TRACE(DL_ERROR, "%s: Cannot process cmd=%d, module is not enabled\n", __func__, cmd);
        return -EPERM;
    }
        ...
        (cmd == CB_DRIVER_REQUEST_SET_TRUSTED_PATH))
    {
        ...
        if (!page)
        {
            TRACE(DL_ERROR, "%s: alloc failed cmd=%d\n", __func__, cmd);
            return -ENOMEM;
        }
    } else
    {
        ...
        {
            TRACE(DL_ERROR, "%s: failed to copy arg\n", __func__);
            return -ENOMEM;
        }
    }
    ...
    case CB_DRIVER_REQUEST_ACTION:
        {
            ...
            {
                TRACE(DL_ERROR, "%s: Module state is %d, cmd %d, action %d is illegal\n", __func__, moduleState, cmd, action);
                return -EPERM;
            }
