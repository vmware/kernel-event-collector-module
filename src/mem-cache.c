...
int cb_mem_cache_show(struct seq_file *m, void *v)
{
    ...
    seq_printf(m, "Generic Memory Buffers: %ld\n", atomic64_read(&s_cb_mem_cache.generic_buffer_count));
    seq_printf(m, "Generic Memory Size: %ld\n", atomic64_read(&s_cb_mem_cache.generic_buffer_size));
    seq_puts(m, "\n");
    ...
    list_for_each_entry(cache, &s_cb_mem_cache.list, node) {
            ...
            seq_printf(m, "%40s | %6ld | %40s | %9d |\n",
                       cache->name,
                       atomic64_read(&(cache->allocated_count)),
                       cache_name,
                       cache_size);
    }
    cb_write_unlock(&s_cb_mem_cache.lock, &context);
...
void *__cb_mem_cache_alloc_generic(size_t size, ProcessContext *context, bool doVirtualAlloc, const char *fn, uint32_t line)
{
    void             *new_allocation   = NULL;

    if (size > 0)
    {
        size          += GENERIC_BUFFER_SZ;

        if (!doVirtualAlloc)
        {
            new_allocation = kmalloc(size, GFP_MODE(context));
        } else if (doVirtualAlloc && IS_NON_ATOMIC(context))
        {
            new_allocation = vmalloc(size);
        } else
        {
...
char *cb_mem_cache_strdup(char *src, ProcessContext *context)
{
    char *dest = NULL;
