...
int _socket_recvmsg_hook_counted(ProcessContext *context, struct socket *sock, struct msghdr *msg, int size, int flags)
{
    ...
    // TODO: We may never see a CMSG here.  See udp_configure_raddr
    if (proto == IPPROTO_UDP)
    {
        ...
        struct msghdr msghdr_kernel;
        int ret;

        oldfs = get_fs();
        set_fs(get_ds());
        ret = copy_from_user(&msghdr_kernel, msg, sizeof(msghdr_kernel));
        set_fs(oldfs);

        TRY_STEP(SKIP_CMSG, !ret);
        TRY_STEP(SKIP_CMSG, msghdr_kernel.msg_controllen > 0);

        cmsg_kernel = cb_mem_cache_alloc_generic(msghdr_kernel.msg_controllen, context);
        TRY_STEP(SKIP_CMSG, cmsg_kernel);

        oldfs = get_fs();
        set_fs(get_ds());
        ret = copy_from_user(cmsg_kernel, msghdr_kernel.msg_control, msghdr_kernel.msg_controllen);
        set_fs(oldfs);
        TRY_STEP(SKIP_CMSG, !ret);

        // after recvmsg(sd, &msg, flags);
        for (cmsg = CMSG_FIRSTHDR(&msghdr_kernel); cmsg; cmsg = CMSG_NXTHDR(&msghdr_kernel, cmsg))
        {
            if (!cmsg->cmsg_len)
            {
                break;
            ...
            case IP_PKTINFO:
                ...
                localAddr.as_in4.sin_addr = pktinfo->ipi_addr;
                break;
            case IPV6_PKTINFO:
                pktinfo6 = (struct in6_pktinfo *)CMSG_DATA(cmsg);
                memcpy(&localAddr.as_in6.sin6_addr, &pktinfo6->ipi6_addr, sizeof(pktinfo6->ipi6_addr));
                break;
            default:
                break;
            }
...
CATCH_SKIP_CMSG:
    // If we have an issue collecting the cmsg data we want to send the event with the data we have.
